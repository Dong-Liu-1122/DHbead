...
enum E_GF2N_RETURNVALUE scalar_mul(gf2n_t output, gf2n_t x_point, gf2n_t scalar)
{
  ...
  {
    return (enum E_GF2N_RETURNVALUE)ret_val;//增加强制类型转换
  }
...
void gf2n_mul(gf2n_t out, gf2n_t op1, gf2n_t op2)
{
	  ...
	  for (j = 15; j > 3; j--)
	    {
	      for (i = 7; i > -1; i--)
	        if (op1[i] & t) gf2n_sum(temp + i, op2);
	      
	      /* leftshift */
	      for (i = 16; i > 0; i--)
	        temp[i] = ((temp[i] << 1) | (temp[i - 1] >> 15));
	      temp[0] <<= 1;
	  ...
	  for (i = 8; i > -1; i--)
	    if (op1[i] & 1) gf2n_sum(temp + i, op2);
...
void gf2n_square(gf2n_t out, gf2n_t op)
{
  ...
  static const UWORD square_tab[256] = {
  ...
  t = op[0];
  temp[0] = square_tab[t & 0xFF];
  temp[1] = square_tab[((t >> 8)&0xff)];
  t = op[1];
  temp[2] = square_tab[t & 0xFF];
  temp[3] = square_tab[((t >> 8)&0xff)];
  t = op[2];
  temp[4] = square_tab[t & 0xFF];
  temp[5] = square_tab[((t >> 8)&0xff)];
  t = op[3];
  temp[6] = square_tab[t & 0xFF];
  temp[7] = square_tab[((t >> 8)&0xff)];
  t = op[4];
  temp[8] = square_tab[t & 0xFF];
  temp[9] = square_tab[((t >> 8)&0xff)];
  t = op[5];
  temp[10] = square_tab[t & 0xFF];
  temp[11] = square_tab[((t >> 8)&0xff)];
  t = op[6];
  temp[12] = square_tab[t & 0xFF];
  temp[13] = square_tab[((t >> 8)&0xff)];
  t = op[7];
  temp[14] = square_tab[t & 0xFF];
  temp[15] = square_tab[((t >> 8)&0xff)];
  t = op[8];
  temp[16] = square_tab[t & 0xFF];
