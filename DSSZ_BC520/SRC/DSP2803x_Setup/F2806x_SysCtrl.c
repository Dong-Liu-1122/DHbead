...
void InitSysCtrl(void)
{
    ...
    // IntOsc1Sel();
    EALLOW;
    SysCtrlRegs.CLKCTL.bit.OSCCLKSRC2SEL = 0; //"0"选择外部振荡器; 1:选择内部振荡器
    SysCtrlRegs.CLKCTL.bit.OSCCLKSRCSEL  = 1; //“1”选择外部晶振输入，“0” Clk Src = INTOSC1选择内部时钟源1
    SysCtrlRegs.CLKCTL.bit.XTALOSCOFF    = 0; // 启动内部振荡器
    SysCtrlRegs.CLKCTL.bit.INTOSC1OFF    = 1; // Turn off INTOSC1  关闭内部时钟源1，内部时钟源1（10MHz）
    SysCtrlRegs.CLKCTL.bit.INTOSC2OFF    = 1; // Turn off INTOSC2  关闭内部时钟源2
    SysCtrlRegs.CLKCTL.bit.XCLKINOFF     = 1; // Turn off XCLKIN 关闭外部时钟输入
    EDIS;
...
void MemCopy(Uint16 * restrict SourceAddr, Uint16* restrict SourceEndAddr, Uint16* restrict DestAddr)
{
